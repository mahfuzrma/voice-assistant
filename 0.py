...
@author: Mahfuz """
import os
import winsound #import tkMessageBox
import pyttsx3
import pymysql
from tkinter import *
import speech_recognition as sr
db = pymysql.connect(user="root",password="", host="localhost",database="m")
#\l "prepare a cursor object using cursor() method
cursor = db.cursor()
engine = pyttsx3.init()
voices=engine.getProperty('voices')
engine.setProperty('voice',voices[1].id ) #engine.sroot() winsound.MessageBeep() engine.say("welcome to voice command app") engine.runAndWait()
root = Tk() #to = Tk()
root.title('Voice Command Application')
root.geometry('350x300+700+10') #root.title('Output') #global c
c=''
print("opened")
engine.say("welcome to voice command Application") 
engine.runAndWait() 
def wikit():
    import wikipedia
    command =E2.get()
...
def OpenTask():
   ...
   # Prepare SQL query to INSERT a record into the database.
    ...
    # Now print fetched result
        ...
        engine.runAndWait()        
def voice():
    global c #L9 = Label(root, text="speak") #L9.pack( side = TOP) ''' to = Tk() L9 = Label(to, text="speak") L9.pack( side = TOP) ''' 
    recording = sr.Recognizer() 
    with sr.Microphone() as source:
        recording.adjust_for_ambient_noise(source) #engine = pyttsx3.init()
...
def reopen(): 
    ...
    with sr.Microphone() as source:
        recording.adjust_for_ambient_noise(source) #engine = pyttsx3.init() 
        engine.say("Want to continue") #engine.setProperty('rate',120) #engine.setProperty('volume', 0.8) 
        engine.runAndWait() 
        print("Speak now") #global c 
        audio = recording.listen(source) #print("said") 
    try: 
        global c 
        c=recording.recognize_google(audio) 
        print("You said: \n" + c)
...
def voicef(): 
    ...
    #engine = pyttsx3.init() engine.say("speak now") #engine.setProperty('rate',120) #engine.setProperty('volume', 0.8) engine.runAndWait() print("Speak now") #global c
        audio = recording.listen(source) #print("said")
    try:
        global c
...
def ad():
    ...
    roo.mainloop
    '''
    def donothing(): print("m") filewin = Toplevel(root) button = Button(filewin, text="Do nothing button") button.pack() ''' 
...
def About(): 
    ...
    L3.pack( side = BOTTOM) 
    ro.mainloop
#print('Text Command') #os.startfile('A:/Cse 499/m.m4a') #L2 = Label(root, text="Voice Command Window") #L2.pack( side = TOP) L2 = Label(root, text="USER PANEL") L2.pack( side = TOP) L2 = Label(root, text="Text Command") L2.pack( side = TOP)
E1 = Entry(root, bd =5) 
E1.pack()
